...
function downloadvideo(servername, streamname, stateid, cur_stateid, isdwn) {
 ...
 function downloadvideobydate(servername, streamname, stateid, cur_stateid, isdwn, seldate) {
    ...
    //https://eletn21blob1.blob.core.windows.net/etv1-container/29-03-2021/000077.flv
            //https://eletn21blob1.blob.core.windows.net/etv1-container/29-03-2021/013657.flv
    dwnlnk = "https://eletn21blob1.blob.core.windows.net/" + servername + "-container/"+ (("0" + mydate.getDate()).slice(-2))+ "-" +(("0" + (mydate.getMonth()+1)).slice(-2))+ "-" + mydate.getFullYear() +"/" + streamname + ".flv";
...
function loadhlsplayer(playerid, sname, sid, cur_stateid) {
	...
	var hlsurl = "http://" + sname + '/live-record/' + vidname + "/index.m3u8";
...
function LoadlivePlayer6(playerid, sname, plid) {
    ...
    jwplayer(vidname).setup({
        ...
        sources: [{
            ...
            {
                file: "http://" + sname + ":8080/live-record/" + playerid + "/index.m3u8",
                type: "hls"
            }
],
...
function LoadlivePlayer7(playerid, sname, plid) {
	...
	var hlsurl = "https://" + sname + '/live-record/'+playerid+"/index.m3u8";
	//alert(hlsurl + "==Priya")
    ...
    if (/Android/i.test(navigator.userAgent)) {
       ...
       var vidname = playerid;
        if (plid == "mplayer") {
            ...
            $('#btnsettings').show();
            $("#playerimg").hide();
            $('#schloc').html("<h5><span style='color:#337AB7;'><b>Camera No/AC/PS : </b></span>" + playerid +"/"+ schname + " / " + locname + "<span style='color:#337AB7;'><b>Location : </b></span>" + location + "</h5>");
        ...
        if (playerType == "JWPlayer") {
            ...
            //$('#RTMPplayer').hide();
           // alert("priya" + playerid + "==" + sname + "==" + plid);
