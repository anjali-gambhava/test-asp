...
namespace exam
{
    public partial class ChannelView1 : System.Web.UI.Page
    {
        public int stateid
        {
            get
        ...
        {
            db_data dbContext = new db_data();
            var vehicleDataList = dbContext.GetVehicleData(ddlDistrict.SelectedValue, ddlAssembly.SelectedValue).Tables[0].AsEnumerable().ToList();

            var assemblyWizeTotle = dbContext.GetAssemblyWizeTotleVehicalData().Tables[0].AsEnumerable().ToList();
